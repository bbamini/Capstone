...
# Read the texts and generate a corpus
alltexts <- readtext("C:/Users/bamini/Documents/Coursera/Capstone/final/en_US", encoding = "UTF-8")
corp <- corpus(alltexts)

sample25 <- readtext("C:/Users/bamini/Documents/Coursera/Capstone/final/en_US/sample25.txt", encoding = "UTF-8")
corp25 <- corpus(sample25)

# Here are the bad words:
...
# Basics of the corpus
summary(corp)

# Tokenize the corpus while removing numbers, punctuations, symbols, twitter handles and urls
tok <- tokens(corp, what="word", remove_numbers = TRUE, remove_punct = TRUE, remove_symbols = TRUE, 
              remove_twitter = TRUE, remove_url = TRUE)

tok25 <- tokens(corp25, what="word", remove_numbers = TRUE, remove_punct = TRUE, remove_symbols = TRUE, 
              remove_twitter = TRUE, remove_url = TRUE)


# Removing bad word tokens
tok_noBad <- tokens_remove(tok, badWords)
tok25_clean <- tokens_remove(tok25, badWords)


# Converting tokenized words to a document feature matrix and listing top ten features
unigramdfm <- dfm(tok_noBad)
topfeatures(docmatrix)

# Removing stopwords and listing top ten features
nostop <- dfm(tok_noBad, remove = stopwords("english"))
topfeatures(nostop)
...
tetragram25 <- tokens_ngrams(tok25_clean, n = 4, concatenator = " ")
tetragram25dfm <- dfm_trim(dfm(tetragram25))

pentagram25 <- tokens_ngrams(tok25_clean, n = 5, concatenator = " ")
pentagram25dfm <- dfm_trim(dfm(pentagram25))

# Convert to data.table

gram1 <- convert(unigramdfm, to = "data.frame")
test <- data.frame(names(gram1))
gram1 <- mutate(test, counts = colSums(gram1))

gram2 <- convert(bigramdfm, to = "data.frame")
test <- data.frame(names(gram2))
gram2 <- mutate(test, counts = colSums(gram2))
gram2 <- data.table(gram2)
gram2 <- gram2[,c("base", "predicted") := tstrsplit(names.gram2., " ", fixed = TRUE)][, 2:4]

gram3 <- convert(trigramdfm, to = "data.frame")
test <- data.frame(names(gram3))
gram3 <- mutate(test, counts = colSums(gram3))
gram3 <- data.table(gram3)
gram3 <- gram3[,c("v1", "v2", "predicted") := tstrsplit(names.gram3., " ", fixed = TRUE)][, 2:5]
gram3 <- mutate(gram3, base = paste(v1, v2, " "))[,c(1, 4, 5)]


gram4 <- convert(tetragram25dfm, to = "data.frame")
gram4 <- data.table(gram4)
test <- data.table(names(gram4))
...
gram5 <- convert(pentagram25dfm, to = "data.frame")
