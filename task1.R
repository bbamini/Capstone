# This is an attempt at writing a function to tokenize and a function to remove bad words from a text.
# This is done without relying on NLP packages


setwd("C:/Users/bamini/Documents/Coursera/Capstone/final/en_US")
library(stringr)

sample <- readLines("C:/Users/bamini/Documents/Coursera/Capstone/sample.txt", encoding = "UTF-8")

tokenize <- function(rawLines) {
  # Bad Characters include punctuations (excluding apostrophes and number digits)
  badChars <- "[.]|[,]|[#]|[$]|[%]|[&]|[*]|[+]|[-]|[/]|[:]|[;]|[<]|[=]|[>]|[?]|[@]|[_]|[`]|[~]|[!]|[{]|[}]|[|]|[(]|[)]|[[]|[]]|[[:digit:]]"
  # The iteration removes bad characters from each line
  cleanedLines <- sapply(rawLines, function(line) gsub(badChars, "", line))
  cleanedLines <- str_trim(cleanedLines) # Removes excess spaces from each line
  splits <- strsplit(cleanedLines, "[[:blank:]]+") # splits into words by separating at spaces or tabs
  splits 
}
...
removeDirtyLines <- function(rawLines) {
  # List of bad words courtesy of https://github.com/web-mech/badwords-list/blob/master/lib/array.js
  ...
  checkList <- sapply(badWords, grepl, rawLines)
  goodLineIndex <- apply(checkList, 1, sum)
  goodLines <- rawLines[which(goodLineIndex %in% 0)]
}
...
cleanblogs <- removeDirtyLines(blogs)
cleantweets <- removeDirtyLines(twitter)
cleannews <- removeDirtyLines(news)
